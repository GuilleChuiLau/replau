...
def ingredient_ledger_page(request:Request,flash:str="",x_admin_token:Optional[str]=Header(default=None,alias="X-Admin-Token")) -> HTMLResponse:
    ...
    usage_variance=pg_get("/v_ingredient_usage_variance_daily?select=*&order=usage_date.desc,ingredient_name.asc&limit=300")
    order_consumption=pg_get("/v_order_ingredient_consumption?select=*&order=batch_id.desc&limit=100")
    ...
    variance_rows="".join(f'''<tr class="{'warn' if float(v.get('waste_rate_pct') or 0)>5 else ''}"><td>{esc(v.get('usage_date'))}</td><td>{esc(v.get('ingredient_name'))}<br><span class="muted">{esc(v.get('sku') or '—')}</span></td><td>{esc(v.get('warehouse_code'))}</td><td>{esc(v.get('theoretical_consumption_kg'))} kg</td><td>{esc(v.get('waste_variance_kg'))} kg</td><td>{esc(v.get('actual_usage_kg'))} kg</td><td>{esc(v.get('waste_rate_pct'))}%</td><td>S/ {float(v.get('waste_cost') or 0):.2f}</td></tr>''' for v in usage_variance)
    consumption_rows="".join(f'''<tr class="{'warn' if int(c.get('skipped_line_count') or 0)>0 else ''}"><td>{esc(c.get('posted_at'))}</td><td>{esc(c.get('pedido_num'))}</td><td>{esc(c.get('warehouse_code'))}</td><td>{esc(c.get('recipe_line_count'))}/{esc(c.get('order_line_count'))}</td><td>{esc(c.get('skipped_line_count'))}</td><td>{esc(c.get('movement_count'))}</td><td>{esc(c.get('total_quantity_kg'))} kg</td><td>S/ {float(c.get('theoretical_cost') or 0):.2f}</td><td>{esc(c.get('actor'))}</td></tr>''' for c in order_consumption)
    ...
    variance_card=f'''<div class="card"><h2>Usage and waste variance</h2><p class="muted">Theoretical usage comes from recipes posted at dispatch. Actual usage adds recorded waste; the difference is the waste variance and its cost impact.</p><table><thead><tr><th>Date</th><th>Ingredient</th><th>Warehouse</th><th>Theoretical</th><th>Waste variance</th><th>Actual usage</th><th>Waste rate</th><th>Waste cost</th></tr></thead><tbody>{variance_rows or '<tr><td colspan="8">No recipe consumption or waste has been posted.</td></tr>'}</tbody></table></div>'''
    consumption_card=f'''<div class="card"><h2>Automatic order consumption</h2><p class="muted">Each dispatch is posted once. Rows highlighted below contain order lines without a complete active recipe; those lines are reported but do not block rollout.</p><table><thead><tr><th>Posted</th><th>Order</th><th>Warehouse</th><th>Recipe lines</th><th>Skipped</th><th>Movements</th><th>Consumed</th><th>Theoretical cost</th><th>Actor</th></tr></thead><tbody>{consumption_rows or '<tr><td colspan="9">No dispatched recipe consumption yet.</td></tr>'}</tbody></table></div>'''
    body=body.replace('<div class="card"><h2>Movement history',variance_card+consumption_card+'<div class="card"><h2>Movement history',1)
