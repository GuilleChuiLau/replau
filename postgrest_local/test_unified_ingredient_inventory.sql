...
DO $$
DECLARE
 warehouse_id integer;supplier_id integer;product_id integer;ingredient_id integer;recipe_id integer;result jsonb;stock_qty numeric;cost_row record;
BEGIN
 ...
 VALUES('CONTRACT INGREDIENT','CONTRACT-ING',supplier_id,10,'PEN',2,true) RETURNING id INTO ingredient_id;
 result:=api.post_ingredient_receipt(ingredient_id,warehouse_id,10,20,'PEN',supplier_id,'contract-receiver','Rollback supplier receipt','LOT-CONTRACT',current_date+30,'CONTRACT-RECEIPT');
 ...
 BEGIN
  PERFORM api.post_ingredient_waste(ingredient_id,warehouse_id,9,'contract-kitchen','DAMAGE','Expected insufficient stock test');
 ...
 END;
 INSERT INTO api.recetas_costeo(producto_id,nombre,rendimiento_unidades) VALUES(product_id,'CONTRACT RECIPE',2) RETURNING id INTO recipe_id;
 INSERT INTO api.receta_ingredientes_costeo(receta_id,ingrediente_id,cantidad_g) VALUES(recipe_id,ingredient_id,500);
 SELECT * INTO cost_row FROM api.v_receta_costos WHERE receta_id=recipe_id;
 ...
 IF (SELECT count(*) FROM api.ingredient_stock_movements WHERE ingredient_id=ingredient_id)<>4 THEN RAISE EXCEPTION 'Expected four immutable ingredient movements';END IF;
END $$;
