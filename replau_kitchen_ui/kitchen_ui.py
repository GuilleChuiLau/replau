...
def dashboard(request: Request) -> HTMLResponse:
    ...
    queue_counts = {
        ...
        "ready": len([o for o in orders if str(o.get("kitchen_status") or "").upper() == "LISTO"]),
        "late": len([o for o in orders if str(o.get("queue_color") or "").upper() in {"YELLOW", "RED"}]),
        "urgent": len([o for o in orders if str(o.get("queue_color") or "").upper() == "RED"]),
    }
    next_orders = sorted(
        orders,
        key=lambda o: (
            {"RED": 0, "YELLOW": 1, "GREEN": 2}.get(str(o.get("queue_color") or "").upper(), 3),
            -numeric_value(o.get("queue_minutes")),
            str(o.get("pedido_num") or ""),
        ),
    )[:4]
    next_html = "".join(
        f"""<div class="workspace-row"><strong>{esc(o.get('pedido_num'))}</strong><span>{esc(o.get('cliente_nombre'))}</span><b>{esc(o.get('queue_minutes'))} min</b></div>"""
        for o in next_orders
    ) or '<div class="workspace-empty">Sin pedidos activos.</div>'
    workspace_html = f"""
    <section class="workspace-panel">
      <div class="workspace-head">
        <div>
          <h2>Kitchen Workspace</h2>
          <p>Prioridad de producción, presión de tiempo y pedidos listos para pasar a picking.</p>
        </div>
        <a class="button secondary" href="http://127.0.0.1:8790/ops/picking">Abrir Picking</a>
      </div>
      <div class="workspace-grid">
        <div><span>En cola</span><strong>{queue_counts["total"]}</strong></div>
        <div><span>En preparación</span><strong>{queue_counts["prep"]}</strong></div>
        <div><span>Listos</span><strong>{queue_counts["ready"]}</strong></div>
        <div><span>Tarde / urgente</span><strong>{queue_counts["late"]} / {queue_counts["urgent"]}</strong></div>
      </div>
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing:border-box; }}
    ...
    .workspace-grid {{ display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); gap:12px; margin-bottom:14px; }}
    .workspace-grid div {{ background:#0b1220; border:1px solid rgba(139,92,246,.24); border-radius:16px; padding:14px; }}
    .workspace-grid span {{ display:block; color:var(--muted); font-size:12px; font-weight:850; text-transform:uppercase; letter-spacing:.06em; }}
    .workspace-grid strong {{ display:block; margin-top:8px; font-size:28px; line-height:1; }}
    .workspace-next {{ display:grid; gap:8px; }}
    .workspace-row {{ display:grid; grid-template-columns:120px 1fr auto; gap:10px; align-items:center; background:rgba(255,255,255,.045); border:1px solid rgba(255,255,255,.08); border-radius:14px; padding:10px 12px; }}
    .workspace-row span {{ color:#cbd5e1; }}
    .workspace-row b {{ color:#ddd6fe; }}
    .workspace-empty {{ color:var(--muted); padding:8px 0; }}
