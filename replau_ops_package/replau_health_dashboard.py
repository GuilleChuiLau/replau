...
def collect():
    ...
    stuck_classification=classify_whatsapp_rows(stuck,policy)
    error_classification=classify_whatsapp_rows(errors["data"] if errors["ok"] else [],policy)
    crit=[]; warn=[]
    for s in services:
    ...
    if stuck_classification["actionable"]: crit.append(f"Stuck WhatsApp outbox rows: {len(stuck_classification['actionable'])}")
    if error_classification["actionable"]: crit.append(f"ERROR WhatsApp outbox rows: {len(error_classification['actionable'])}")
    ...
    policy["acknowledged_historical_stuck_ids"]=[row.get("id") for row in stuck_classification["historical"]]
    policy["acknowledged_historical_error_ids"]=[row.get("id") for row in error_classification["historical"]]
    if policy.get("state")=="TRIPPED": crit.append(f"WhatsApp outbound circuit tripped: {policy.get('state_reason')}")
    elif policy.get("state")=="PAUSED": warn.append(f"WhatsApp outbound intentionally paused: {policy.get('state_reason') or 'operator safety pause'}")
    ...
    if not w["ok"]:
        ...
        if policy.get("state")=="PAUSED":
            warn.append("WhatsApp gateway is connected; outbound delivery remains paused or restricted")
        else:
...
<div class="card"><h2>Error WhatsApp Outbox</h2><p class="muted">Historical failures predating an explicit safety pause remain visible here and in the policy card, but only new actionable failures affect health.</p>{tbl(h["error_outbox"]["data"],["id","pedido_num","event_type","status","attempts","last_attempt_at","created_at","error_message"])}</div>
